...
**/
function streamAssetsForImport(sourceUrls, targetSite, {concurrency, overwriteLayouts, ignoreLayouts, headers} = {}) {
  // stream Clay assets ({url, data, isLayout}) for each url
  return clayInput.streamAssets(sourceUrls, concurrency, headers)
    // replace prefixes in assets
    .filter(ignoreLayouts ?
      asset => !asset.isLayout :
      () => true
    )
    ...
    .through(stream => overwriteLayouts ?
      stream :
      skipLayoutCmpts(stream, {concurrency, headers})
    )
...
**/
function skipLayoutCmpts(stream, {concurrency, headers}) {
  return stream
    ...
    .flatMap(asset => asset.overwrite === false ?
      checkExisting(asset, {concurrency, headers}) :
      h.of(asset)
    );
}
 ...
 * @return {Stream}
 */
function checkExisting(asset, {concurrency, headers} = {}) {
  return rest.get(asset.url, {concurrency, type: 'json', headers})
 ...
 * @param {object} [opts.headers] Custom headers for requests to target site
 * @param {boolean} [opts.overwriteLayouts] Overwrite layouts and their children
 * @param {function} [opts.touchup] Function to modify assets before PUT reqs
 * @return {Stream}
 */
function importUrl(sourceUrl, targetSite, {concurrency, key, getHeaders, putHeaders, overwriteLayouts, ignoreLayouts, touchup} = {}) {
  return streamAssetsForImport(sourceUrl, targetSite, {concurrency, headers: getHeaders, overwriteLayouts, ignoreLayouts})
    .flatMap(asset => importApi.importAsset(asset, {key, concurrency, headers: putHeaders, touchup}));
}
